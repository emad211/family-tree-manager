...
class FamilyTreeCLI:
    ...
    def run(self):
        while True:
            ...
            print("3. Check Ancestor")
            print("4. Check Common Ancestor")
            print("5. Check if Siblings")
            print("6. Check Furthest Descendant")
            print("7. Check Furthest Relationship")
            print("8. Exit")
            ...
            if choice == '1':
                ...
                self.check_common_ancestor()
            elif choice == '5':
                self.check_are_siblings()
            elif choice == '6':
                self.check_furthest_descendant()
            elif choice == '7':
                self.check_furthest_relationship()
            elif choice == '8':
                break
            else:
    ...
    def check_ancestor(self):
        descendant_name = input("Enter the name of the descendant: ")
        ancestor_name = input("Enter the name of the ancestor: ")
        is_ancestor = self.family_tree.is_ancestor(hash_name(ancestor_name), hash_name(descendant_name))
        print(f"Is {ancestor_name} an ancestor of {descendant_name}: {is_ancestor}")

    def check_common_ancestor(self):
        name1 = input("Enter the name of the first person: ")
        name2 = input("Enter the name of the second person: ")
        common_ancestor_hash = self.family_tree.common_ancestor(hash_name(name1), hash_name(name2))
        common_ancestor_name = "Unknown" if common_ancestor_hash is None else common_ancestor_hash
        print(f"Common ancestor for {name1} and {name2}: {common_ancestor_name}")

    def check_are_siblings(self):
        name1 = input("Enter the name of the first person: ")
        name2 = input("Enter the name of the second person: ")
        are_siblings = self.family_tree.are_siblings(hash_name(name1), hash_name(name2))
        print(f"Are {name1} and {name2} siblings: {are_siblings}")

    def check_furthest_descendant(self):
        name = input("Enter the name to check for furthest descendant: ")
        furthest_depth = self.family_tree.furthest_descendant(hash_name(name))
        print(f"Furthest descendant from {name}: Generation {furthest_depth}")

    def check_furthest_relationship(self):
        furthest_members_hash = self.family_tree.furthest_relationship()
        if furthest_members_hash == (None, None):
            print("No relationship found.")
        else:
            member1, member2 = furthest_members_hash
            print(f"Furthest relationship between {member1} and {member2}")
